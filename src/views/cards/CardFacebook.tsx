...
const CardFacebook = ({name, motivation, hobbies, industry, prevIndustry, portfolio}:{industry?:string; prevIndustry?: string; name?: string, hobbies?: string; motivation?: string, portfolio?: string}) => {
  return (
    <Card sx={{ border: 0, boxShadow: 0, color: 'common.white', backgroundColor: 'primary.main' }}>
      
      <CardContent sx={{ padding: theme => `${theme.spacing(3.25, 5, 4.5)} !important` }}>
      <Box sx={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between' }}>
          <Box sx={{ mr: 2, display: 'flex', flexDirection:"column",  alignItems:"start" }}>
            {/* <Avatar alt='Eugene Clarke' src='/images/avatars/1.png' sx={{ width: 34, height: 34, marginRight: 2.75 }} /> */}
            <Typography variant='h5' sx={{ color: 'common.white' }}>
             <b> {name}</b>
            </Typography>
            {portfolio && <Typography variant='h6' sx={{ color: 'common.white' }}>
             <b> <a rel="noreferrer" target="_blank" href={"http://"+portfolio}>My Portfolio</a></b>
            </Typography>}
          </Box>
        ...
        <Typography variant='body2' sx={{ marginBottom: 3, color: 'common.white' }}>
         ...
         <br/>
         <br/>
         {hobbies && ` My interests and hobbies: ${hobbies}`}  <br/>
         <br/>
         {industry && ` My desired industry: ${industry}`}  <br/>
         <br/>
